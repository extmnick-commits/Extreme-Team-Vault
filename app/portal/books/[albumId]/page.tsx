...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const album = findGroup(await getLibrary('books'), albumId)
  return { title: `${album?.name ?? 'Books'} | Extreme Team Vault` }
}

export default async function BookAlbumPage({ params }: Props) {
  const { albumId } = await params
  const view = await getLibrary('books')
    ...
    <div className="flex flex-col gap-6 sm:gap-8">
      ...
      <header className="flex items-center gap-4 sm:items-end sm:gap-6">
        <div className="flex size-16 shrink-0 items-center justify-center rounded-2xl border border-line bg-linear-to-br from-sky-50 via-white to-violet-50 shadow-sm sm:size-32">
          <BookOpen className="size-7 text-sky-500 sm:size-14" strokeWidth={1.5} aria-hidden="true" />
        </div>
